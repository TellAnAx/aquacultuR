...
#' period of interest. 
#' @param ag numeric value; absolute growth in grams during the experimental period.
...
rfr <- function(m_feed = NULL, 
  ...
  stopifnot("Error: Inputs must be numeric!" = is.numeric(m_feed), is.numeric(ag), is.numeric(duration))
  ...
  if (any(ag == 0 | duration == 0)) {
    stop("Error: 'ag' and/or 'duration' is equal to zero. The result cannot be calculated.")
  }
